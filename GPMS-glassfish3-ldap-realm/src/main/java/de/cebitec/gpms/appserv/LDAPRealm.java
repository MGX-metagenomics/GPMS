 ...
 *
 * <P>
 * See LDAPLoginModule documentation for more details on the operation of the
 * LDAP realm and login module.
 *
 * <P>
 ...
 *
 * <P>
 * Besides JDK Context properties start with java.naming, javax.security, one
 * can also set connection pool related properties starting with
 * com.sun.jndi.ldap.connect.pool. See
 * http://java.sun.com/products/jndi/tutorial/ldap/connect/config.html for
 * details. Also, the following optional attributes can also be specified:
 * <ul>
 * <li>search-filter - LDAP filter to use for searching for the user entry based
 * on username given to iAS. The default value is <code>uid=%s</code> where %s
 * is expanded to the username.
 * <li>group-base-dn - The base DN to use for group searches. By default its
 * value is the same as base-dn.
 * <li>group-search-filter - The LDAP filter to use for searching group
 * membership of a given user. The default value is <code>
 *       uniquemember=%d</code> where %d is expanded to the DN of the user found by
 * the user search.
 * <li>group-target - The attribute which value(s) are interpreted as group
 * membership names of the user. Default value is <code>cn</code>.
 * <li>search-bind-dn - The dn of ldap user. optional and no default value.
 * <li>search-bind-password - The password of search-bind-dn.optional and no
 * default value.
...
public final class LDAPRealm extends AppservRealm {
    ...
    /**
     * Returns the name of all the groups that this user belongs to. Note that
     * this information is only known after the user has logged in. This is
     * called from web path role verification, though it should not be.
     *
    ...
    public Enumeration<String> getGroupNames(String username) throws InvalidOperationException, NoSuchUserException {
        List<String> v = groupCache.get(username);
        if (v == null) {
    ...
    private void setGroupNames(String username, String[] groups) {
        List<String> v = new ArrayList<>(groups.length);
     ...
     *
     */
    private List<String> dynamicGroupSearch(DirContext ctx, String baseDN,
